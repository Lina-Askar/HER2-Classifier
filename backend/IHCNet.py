...
BCI_ROOT = "/kaggle/input/bci-ihc/bci"
...
def strict_split_items():
    ...
    print("Aggregating & STRICT splitting per-class (test first, then val)...")
    for c in CLASS_DIRS:
        idx = CLASS_TO_IDX[c]
        ...
        all_unique = list(dict.fromkeys(img_train + img_val + img_test + img_rest))  
        ...
        pref_val = [
            ...
            [p for p in img_rest  if p in allowed_for_val],
        ]
        ...
        chosen_train = [p for p in all_unique if (p not in chosen_test_set and p not in chosen_val_set)]
        ...
        print(f"  • Class {c}: total={n_total} → test={len(chosen_test)} (~{100*len(chosen_test)/max(1,n_total):.1f}%), "
    ...
    print(f"Final strict split → TRAIN={len(train_items)} | VAL={len(val_items)} | TEST={len(test_items)}")
...
def load_bci_dataset():
    ...
    for label_str, folder_name in BCI_CLASS_DIRS.items():
        idx = int(label_str)   # 0,1,2,3
        class_path = os.path.join(BCI_ROOT, folder_name)
        if not os.path.isdir(class_path):
            print(f"[BCI] Folder {folder_name} not found, skipping.")
            continue
        imgs = list_images_recursively(class_path)
        print(f"[BCI] Class {idx} ({folder_name}): {len(imgs)} images")
        for p in imgs:
            bci_items.append((p, idx))
    print(f"[BCI] Total added to TRAIN = {len(bci_items)} images\n")
    return bci_items
...
def build_loaders():
    ...
    print(f"FINAL TRAIN size  = {len(train_items)}")
    print(f"FINAL VAL size   = {len(val_items)}")
    print(f"FINAL TEST size  = {len(test_items)}\n")
